...
| 23 | Categories + tag/cluster mapping (v2 leftover) | 🟡 | 1 d | **schema + CLI shipped**, UI deferred. v11 migration adds `categories`, `tag_category_map`, `cluster_categories` (FK ON DELETE CASCADE; UNIQUE on tag/cluster id so a thing maps to at most one category). `phototag category add/rm/list/map/unmap` covers the full CLI surface (cluster precedence > tag > manual TODO). `Store.categories_for_image()` resolves the union. `phototag xmp write` now emits `lr:HierarchicalSubject` as `category|subject` per spec, idempotent through the existing mtime+content gate. Remaining: in-app UI for editing rules (deferred per follow-up). |
