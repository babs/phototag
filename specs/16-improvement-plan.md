...
| 4 | Per-identity threshold tuning | 🟢 | 4 h | v10 migration adds `face_identities.{sim_n, sim_mean, sim_M2}`; `Store.record_identity_attach_sim` folds each successful auto-attach via Welford. `attach_face_to_best_identity` widens the auto-validate band by `2σ` of past attach sims (capped at 0.95) once the identity has ≥5 prior attaches — high-variance identities (kid drift, lighting swings) demand more confidence before they auto-validate; low-variance ones keep the global 0.7 default. Surfaced via `phototag faces stats --per-identity`. |
...
| 7 | Constrained HDBSCAN (tier-3 sticky) | 🟢 | 1 d | hand-rolled, no new dependency. `cluster_faces(..., tier3_constraints=True)` (`phototag faces cluster --tier3-constraints`) precomputes a Euclidean distance matrix on the UMAP-reduced space, then surgically rewrites it: must-link pairs (anchor-spoke per `named` name, O(n) edges) → 0.0; cannot-link pairs (`unassigned` against a labelled cluster vs every other face named that label, capped at 50k edges) → 99.0; HDBSCAN runs with `metric='precomputed'`. Tier-1 (replay named) + tier-2 (cannot-link in attach helper) post-passes still run on top — tier-3 nudges *during* clustering, the others enforce *after*. |
...
| 13 | Drag-to-redraw bbox | 🟢 | 4 h | `POST /api/faces/{id}/redraw-bbox` body `{bbox: [x,y,w,h]}`. Backend re-thumbnails the source image to `DETECT_MAX_SIDE`, crops with a 25 % margin, runs the existing `FaceDetector` on the crop (RetinaFace re-aligns + ArcFace re-embeds), picks the highest-`det_score` face, translates the local bbox back to image coords, updates `faces.{bbox_json,embedding,dim,det_score,landmarks_json}` via `Store.update_face_bbox_and_embedding`, and evicts the cached `/face-thumb/{id}` JPEG. Cluster assignment stays untouched (the next attach / verify pass will catch a drift). Frontend: popover gets a "redraw" button (hotkey `B`) that opens an in-place edit mode — drag corners to resize, drag inside to translate; `Enter` saves, `Esc` cancels. Returns 422 when the user-drawn box has no detectable face inside (asks them to widen). |
...
| 20 | `phototag backup` SQLite snapshot | 🟢 | 0.25 h | `phototag backup [--out PATH]` uses `sqlite3.Connection.backup` on a fresh source connection; atomic via `<dst>.tmp` → rename; default dst `<db_path.parent>/backups/phototag-<UTC-iso>.db` |
...
| 23 | Categories + tag/cluster mapping (v2 leftover) | 🟢 | 1 d | v11 migration adds `categories`, `tag_category_map`, `cluster_categories` (FK ON DELETE CASCADE; UNIQUE on tag/cluster id so a thing maps to at most one category). `phototag category add/rm/list/map/unmap` covers the CLI surface (cluster precedence > tag > manual TODO). `Store.categories_for_image()` resolves the union. `phototag xmp write` emits `lr:HierarchicalSubject` as `category|subject`, idempotent through the existing mtime+content gate. UI: third sidebar view `categories` lists categories with rule counts, has an inline "+ new" form; clicking a category renders the workspace rule editor — list of bound tags / clusters, unmap button per row, a tag-bind input with `/api/tags`-sourced datalist autocomplete, and a cluster-id-bind numeric input. Endpoints: `GET/POST /api/categories`, `DELETE /api/categories/{name}`, `GET /api/categories/{name}`, `POST /api/categories/{name}/rules/{tag,cluster}`, `DELETE /api/categories/rules/{tag,cluster}/{key}`. |
| 24 | CI pipeline | 🟢 | 1 h | `.github/workflows/ci.yml` with two jobs: `fast` (push/PR) installs `--extra ui` (test_ui_api needs fastapi at module top) + dev group, runs the project's `pre-commit` hook stack (ruff lint + format + secrets + pyupgrade + mypy) followed by `pytest -m "not slow" --cov`; `slow` (cron `17 3 * * *`) installs `--extra all` and runs `pytest -m slow` with a 60-min timeout. `concurrency.group` cancels superseded PR runs; `permissions: contents: read` keeps the default token minimal. Verified locally with `act -j fast`. |
...
| 27 | Per-image manual category override | 🟢 | 0.5 d | The third (highest-precedence) layer in `specs/08-xmp-categories.md`. v12 migration adds `image_categories(image_id UNIQUE FK→images CASCADE, category_id FK→categories CASCADE)`. `Store.categories_for_image` *short-circuits* on the override (returns only the manual category when set, not the union with rule layers). CLI: `phototag category map/unmap --image PATH --category C` (mutually exclusive with `--tag` / `--cluster`); `category list` exposes the new `image_rules` block. API: `PUT /api/images/{id}/category` body `{category: "name" | null}` to set/clear; `GET /api/images/{id}` surfaces `manual_category`. UI: 📁 chip on the lightbox info bar opens a popover with `/api/categories`-sourced datalist autocomplete. End-to-end XMP test: tag→A + cluster→B + manual→C ⇒ only `C\|<subject>` emitted. |
| 28 | `phototag query` filter parity with `/api/search` | 🟢 | 4 h | `phototag query "TEXT"` now accepts `--tag T` (repeatable, AND), `--person P` (repeatable, AND), `--min-score F` (gates tag filter — CLIP scores aren't calibrated so it's meaningless on the embedding ranking). Both filters mirror the UI's `/api/search` semantics. `--run-id N` is a **CLI-only extension** that scopes the `--person` filter to one `face_run` (pushed into SQL via `Store.search_images_by_persons(..., run_id=N)`); the UI's `/api/search?run_id=` URL param refers to the image-side `cluster_runs` table — different ID namespace, not propagated here. Filters resolve **before** booting the CLIP text encoder so a no-overlap filter short-circuits to `[]` without paying the model-load cost (and lets the filter logic be tested without `[clip]` extras). Filtered ids mask the cosine score matrix to `-inf` so non-allowed images never make the top-K. Tests: short-circuit on unknown tag (no clip needed), short-circuit on unknown person, full clip-backed intersection (skipped when `[clip]` extra isn't installed), `Store.search_images_by_persons(run_id=…)` SQL push-down (no/run-A/run-B/unknown-run). |
| 29 | FakeTagger / FakeEmbedder fixtures | 🟢 | 0.5 d | `tests/fixtures/fakes.py` provides `FakeTagger` (sha256-of-pixels-keyed deterministic tag set + threshold filtering, with optional per-(w,h) overrides) and `FakeEmbedder` (sha256-keyed unit-normalized vectors of configurable `dim`). Both honor the `phototag.models.base.{Tagger,Embedder}` protocols, so `pipeline.{scan_and_tag,embed_all}` consume them as drop-in replacements. **`tests/test_pipeline.py` (+10 tests)**: scan basic round-trip, idempotence on second run, `--force` re-tags everything, `--force-tag` skips re-hash, decode-failure-counted-not-crashed, threshold filtering, embed skip-existing + `--force`, embed unit-norm round-trip, full scan+embed pipeline. **Coverage lift**: `pipeline.py` 0 % → **88 %**, `store.py` 78 % → 81 %, `cli.py` 41 % → 45 %, **TOTAL 51 % → 57 %** (clustering.py and the model wrappers stay at 0 % because they actually need the [cluster] / [ram] / [clip] weights — fakes can't replace UMAP / HDBSCAN inputs). |
