...
| 27 | Per-image manual category override | ⬜ | 0.5 d | The third (highest-precedence) layer alluded to in `specs/08-xmp-categories.md` but never built. Adds an `image_categories(image_id UNIQUE FK→images, category_id FK→categories)` table (v12 migration) so the user can pin a single photo to a category that wins over both tag rules (cluster-id pulled in via face) and cluster rules. CLI: `phototag category map --image PATH --category C`, `category unmap --image PATH`. UI: `categories` chip on the lightbox info bar (shown alongside the existing tag+person chips) that opens a small popover with the same datalist autocomplete the workspace rule editor uses. `Store.categories_for_image` extends to read the override first and short-circuit the union when present. Mirrored end-to-end test against `lr:HierarchicalSubject` (tag→A + cluster→B + manual→C → only `C\|<subject>` emitted). |
| 28 | `phototag query` filter parity with `/api/search` | ⬜ | 4 h | The CLI `query "TEXT"` only accepts `--limit` / `--embedder` today; the UI's `/api/search` already filters by tag, person, min-score, and run id. Plumb the same filters into the CLI: `phototag query "x-ray of a hand" [--tag T]* [--person P]* [--min-score 0.6] [--run-id N] [--limit 30]`. Implementation is mostly hoisting `Store.search_images_by_persons` + the existing tag-AND join into a shared helper that both `cli.query` and `ui.api_search` call, plus a CLI flag layer. Tests: parity matrix (one CLI invocation per filter combo, asserts identical id-set as the `client.get('/api/search?...')` shape). |
| 29 | FakeTagger / FakeEmbedder fixtures | ⬜ | 1.5 d | Three module paths sit at 0 % coverage today (`pipeline.py`, `clustering.py`, `models/{ram,clip}.py`) because end-to-end exercise needs ML weights. A pair of fakes — `FakeTagger` returning canned `(tag, score)` lists per image hash, `FakeEmbedder` returning deterministic seeded vectors — would unlock pipeline-level integration tests (`scan_and_tag` idempotence on second run, `embed_all` skip-existing, batch failure → `failed` counter, `cluster.run` UMAP+HDBSCAN over 100 fake vectors). Lifts overall coverage from 51 % → ~75 %. Keeps the existing `[ram]` / `[clip]` extras as the production path; the fakes live only under `tests/fixtures/`. |
