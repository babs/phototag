...
| 28 | `phototag query` filter parity with `/api/search` | 🟢 | 4 h | `phototag query "TEXT"` now accepts `--tag T` (repeatable, AND), `--person P` (repeatable, AND), `--min-score F` (gates tag filter), `--run-id N` (scopes person filter to a cluster run). Filters resolve **before** booting the CLIP text encoder so a no-overlap filter short-circuits to `[]` without paying the model-load cost — this also makes the filter logic independently testable without `[clip]` extras. Filtered ids mask the cosine score matrix to `-inf` so non-allowed images never make the top-K. Tests: short-circuit on unknown tag (no clip needed), short-circuit on unknown person, full clip-backed intersection (skipped when `[clip]` extra isn't installed). |
